...
def make_listener_spec(index):
    return ['implementation_id', 'rtprint_listener{0}'.format(index),
...
def ListenerInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=make_listener_spec(index))
    manager.registerFactory(profile, Listener, OpenRTM_aist.Delete)
    comp = manager.createComponent("rtprint_listener{0}".format(index))
...
def connect_listener(cmd_path, path, port, options, orb):
    ...
    if result:
        return result
    listener_name = 'rtprint_listener{0}0.rtc'.format(index)
...
def listen_to_port(cmd_path, full_path,
                  options, tree=None):
