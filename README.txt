...
Detailed help for each command is available by executing that command followed
...
RT System management

rtcryo      Save a running RT System into an RtsProfile file, in either XML or
            YAML format.
rtresurrect Restore a complete RT System from an RtsProfile file.
rtstart     Start a system described in an RtsProfile file.
rtstop      Stop a system described in an RtsProfile file.
rtteardown  Remove all connections described in an RtsProfile file.


The RTC Tree
------------

The component management commands operate on the RTC Tree. This is a file
system-like tree built by parsing name servers to find directories, components
and managers. You can treat it exactly the same way as you treat a normal file
system.
...
Usage: Run rtresurrect.py, passing it the location of an RTSProfile
specification file in a format understood by the rtsprofile library (typically
XML or YAML):
...
Usage: Run rtteardown.py, passing it the location of an RTSProfile
specification file in a format understood by the rtsprofile library (typically
XML or YAML):
...
Usage: Run rtcryo.py, passing it the name of the file to save the system to:
...
activating one component after another has completed activation). For more
information on controlling the ordering, see "State-change plans," below.

Usage: Run rtstart.py, passing it the location of an RTSProfile specification
...
Usage: Run rtstop.py, passing it the location of an RTSProfile specification
file in a format understood by the rtsprofile library (typically XML or YAML):
